...
learning curve). Not a 30+ screen scroll story either — WebSentry is a tool, not a
brochure; a user wants an answer in seconds, not a lingering read.

## Concept

An **inspection dossier**, not a hacker-aesthetic dashboard. WebSentry never attacks —
...
- 30+ screens of scroll narrative — wrong product; WebSentry is a fast tool.
...
- ~~**Entry point:** straight to the scan input, no landing/marketing layer.
  `localhost:3000` *is* the input screen (`/`). Matches "WebSentry is a tool, not a
