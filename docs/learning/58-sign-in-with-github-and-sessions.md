...
Until now, WebSentry had no idea who was asking. Every one of its 21 routes
...
of that gets built, WebSentry needs an answer to "who is this?"
...
someone copies `websentry.db`, they get nothing they can present as a valid
...
- Sign in, then open devtools → Application → Cookies, and look at
  `websentry_session`. Copy the value, sign out, paste it back in as the
...
- Unset `WEBSENTRY_SESSION_SECRET` and hit any protected route — you get a
