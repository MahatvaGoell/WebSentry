...
> **Status:** done. `/audit` shows every plan, pull request, and verification WebSentry
> has ever recorded — the first time any of that history has been read back by anything
> other than a raw SQL query.

## What we built

Since Stage B, every time WebSentry opened a pull request, merged one, or verified a
...
- Sign in, open `/audit` — you'll see every fix WebSentry has ever planned, opened, or
