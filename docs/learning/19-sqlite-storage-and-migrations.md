...
> **Status:** done. Every finished scan is saved to `backend/data/websentry.db` and gets a real `id`.
...
writes the finished report to a real file on disk (`backend/data/websentry.db`)
...
- Run a second scan, then look at both rows at once:
  ```bash
  cd backend && .venv/Scripts/python.exe -c "import sqlite3; c=sqlite3.connect('data/websentry.db'); [print(dict(zip([d[0] for d in c.execute('SELECT * FROM scans').description], r))) for r in c.execute('SELECT * FROM scans')]"
  ```
- Delete `backend/data/websentry.db` entirely and start the server again —
