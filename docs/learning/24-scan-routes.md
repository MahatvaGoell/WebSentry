...
A `layout.tsx` file wraps every page inside its folder. Our scan layout renders a nav bar (the "WebSentry" link and optional breadcrumbs) and then `{children}`, which is whatever page is currently active.
