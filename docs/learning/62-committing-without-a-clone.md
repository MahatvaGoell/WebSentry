...
push, never a force-push, never a write to a branch WebSentry didn't create.
...
This is the only place in WebSentry that writes to somebody's repository, so
...
already have open PRs, WebSentry refuses rather than quietly giving you a PR
...
`websentry/…` branch in someone's repository with nothing explaining what it
is. So:
