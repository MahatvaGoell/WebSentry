...
|---|---|---|
| `WEBSENTRY_SESSION_SECRET` | Yes | Any long random string — sign-in refuses to work without it. |
| `WEBSENTRY_FRONTEND_ORIGIN` | Yes, for a real deploy | Your Vercel URL, e.g. `https://websentry.vercel.app`. Used for the post-sign-in redirect, the CORS allow-list, and whether the session cookie is marked `Secure` — one value now drives all three, so it must be exact (scheme included). |
...
| Variable | Value |
|---|---|
| `NEXT_PUBLIC_API_BASE` | Your Render backend URL, e.g. `https://websentry-api.onrender.com` |
...
- Confirm `WEBSENTRY_FRONTEND_ORIGIN` on Render exactly matches that Vercel
