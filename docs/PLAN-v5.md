# WebSentry v5 — Autofix: deterministic remediation and GitHub pull requests
...
> (`websentry-autofix-4309`), real installation on a throwaway public repo
...
> `websentry/fix-959843b8-1786538197` → `main`, exactly the two planned files
...
WebSentry ends at advice. A finding goes to Groq and comes back as six paragraphs of
...
- `GET /auth/github/install/callback?installation_id&setup_action&state` — verifies
  ...
  that installation, and the `state` match ties that redirect to *this* WebSentry session.
...
`WEBSENTRY_GITHUB_DEV_TOKEN` and **refuses to run unless `WEBSENTRY_ALLOW_DEV_TOKEN=1`**.
...
8. Branch name validated against `^websentry/fix-[0-9a-f]{8}-\d+$` and checked against
...
Stage D lets a user tell WebSentry which repository *serves* a URL scan's site, so
...
- **`next.config.*`** — deliberately narrower, because safely rewriting an arbitrary
  ...
  - one exists **and already defines `headers()`** → declines. Merging into an
    ...
    than WebSentry reformatting a function it can't fully parse.
...
Register the GitHub App (github.com/settings/apps): name "WebSentry Autofix", callback
...
8. `websentry/…` branch created → 9. deterministic patch committed → 10. PR created →
