# WebSentry v2 — Architecture & Incremental Implementation Plan
...
> before any implementation — turn WebSentry from a single-scan tool into
...
| **Self-attested** | WebSentry asks, the user answers, we never test | Input validation, rate limiting, authorization model |
...
### 1.1 — The single biggest change: WebSentry currently has no database
...
`backend/data/websentry.db` exists.
**Verify:** Run a scan → `sqlite3 backend/data/websentry.db "SELECT
...
Nothing from this plan is left outstanding. Any further work on WebSentry —
