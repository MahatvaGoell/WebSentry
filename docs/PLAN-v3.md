# WebSentry v3 — GitHub Repository Scanning
...
WebSentry today scans one thing: a live URL. This adds a second target type —
...
Deliberately *not* in scope: WebSentry never writes to anyone's repository. It
...
`.env` not gitignored, critical CVE present. Plus self-attested rows WebSentry
...
> `MahatvaGoell/WebSentry` scan (138 files): every finding's `file_path`
