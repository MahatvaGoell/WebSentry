...
export async function downloadFixBundle(scanId: string): Promise<void> {
  ...
  link.href = objectUrl;
  link.download = `websentry-fixes-${scanId.slice(0, 8)}.zip`;
  link.click();
...
/* ---------------------------------------------------------------------------
   ...
   Everything above this line either reads data or writes to WebSentry' own
...
/** Stop WebSentry using one installation. Only half a revocation — the App
...
export async function downloadReportPdf(report: ScanReport): Promise<void> {
  ...
  const link = document.createElement("a");
  link.href = objectUrl;
  link.download = `websentry-${host}.pdf`;
