...
WebSentry never attacks the sites it scans. Every check is one of:
...
something harmful, it's out of scope for this project, full stop. WebSentry is
...
Connect a GitHub account and WebSentry can fix some findings itself — not with
...
- **Its own branch, always.** A `websentry/…` branch and one pull request —
  never a commit to the default branch, never a force-push.
- **WebSentry never merges.** The PR is opened; a human reviews it and merges
...
WebSentry was built achievement-by-achievement, each one explained in a
