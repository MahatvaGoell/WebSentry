"""Shared data models for WebSentry.
...
class ChecklistItem(BaseModel):
    ...
    tier determines what produced the state:
      auto         — WebSentry observed this from a finding directly
...
class User(BaseModel):
    """One signed-in person, as WebSentry knows them (PLAN-v5 Stage 0).

    Everything here comes from GitHub — WebSentry stores no password, no email,
...
class GitHubInstallation(BaseModel):
    """One repository-write grant: this user installed the WebSentry GitHub
    App on this GitHub account (PLAN-v5 Stage B).

    Distinct from signing in. Signing in says "GitHub confirms who you are";
    an installation says "you have given WebSentry permission to write to
...
class FixApplyResult(BaseModel):
    ...
    `already_applied` distinguishes "WebSentry just opened this" from "this
