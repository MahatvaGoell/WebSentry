...
user granted WebSentry write access to which GitHub account.
...
`revoked_at` is a *soft* revoke. WebSentry stopping is not the same as GitHub
...
the only half of it WebSentry actually controls.
"""
...
def save_installation(
    ...
    `installation_id` is UNIQUE, so re-installing -- or a different WebSentry
...
def revoke_installation(user_id: int, installation_id: int) -> bool:
    """Mark one of this user's installations as no longer usable by WebSentry.
