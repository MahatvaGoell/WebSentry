"""The one place in WebSentry that writes to somebody's repository.
...
# The only branch names WebSentry is allowed to create. Built by
...
BRANCH_PATTERN = re.compile(r"^websentry/fix-[0-9a-f]{8}-\d+$")
...
def _load_plans(report: ScanReport, finding_keys: list[str]) -> list[tuple[Finding, FixPlan]]:
    ...
    person had the opportunity to look at (CLAUDE.md remediation rule 6,
...
async def _check_drift(
    ...
    """Step 5 — CLAUDE.md remediation rule 7, "Drift aborts".
...
def _check_budget(scan_id: str) -> None:
    ...
    if per_hour >= MAX_PRS_PER_HOUR:
        raise ApplyError(
            f"WebSentry has opened {per_hour} pull requests in the last hour, the limit of "
...
async def apply_fixes(
    ...
    if installation is None:
        raise ApplyError(
            f"WebSentry has no repository access for {owner!r}. Install the WebSentry App "
    ...
    async with httpx.AsyncClient(timeout=20.0) as client:
        ...
        try:
            ...
            # it rather than leaving a stray `websentry/...` branch in
