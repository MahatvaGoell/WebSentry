...
Everything here is scoped by CLAUDE.md's remediation rules 4 and 5: create a
`websentry/…` ref, never touch an existing one, never push to a default
...
class GitHubWriter:
    ...
    def _explain(response: httpx.Response, what: str) -> GitHubWriteError:
        ...
        if status == 403:
            return GitHubWriteError(
                f"The WebSentry App is not permitted to {what} on this repository. "
    ...
    async def create_commit(self, message: str, tree_sha: str, parent_sha: str) -> str:
        ...
        linear commit rather than a merge — WebSentry never merges anything
        (CLAUDE.md remediation rule 5)."""
        response = await self._request(
            "POST",
            "/git/commits",
    ...
    async def create_ref(self, branch: str, commit_sha: str) -> None:
        ...
        the guarantee that WebSentry never moves a branch it did not create is
    ...
    async def delete_ref(self, branch: str) -> bool:
        """Remove a branch WebSentry just created. Best-effort cleanup for the
    ...
    async def create_pull_request(
        ...
        """Open the pull request. `head` is the websentry branch, `base` the
    ...
    async def get_pull_request(self, number: int) -> dict | None:
        ...
        yet" with GitHub's answer rather than with what WebSentry last saw —
