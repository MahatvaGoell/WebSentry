"""How WebSentry gets permission to write to a repository, for one hour at a
...
The chain, and why each link exists:
  ...
  2. WebSentry signs a short **JWT** with it. A JWT is just three
  ...
  3. That JWT proves "I am the WebSentry App" and nothing more. It cannot read
...
def app_jwt(now: int | None = None) -> str:
    """Sign a short-lived assertion that this process is the WebSentry App.
...
class DevTokenProvider(TokenProvider):
    ...
    Two separate variables on purpose. `WEBSENTRY_GITHUB_DEV_TOKEN` holds the
    value; `WEBSENTRY_ALLOW_DEV_TOKEN=1` is a second, deliberate switch that
    ...
    async def token_for(self, client: httpx.AsyncClient, installation_id: int) -> InstallationToken:
        if os.environ.get("WEBSENTRY_ALLOW_DEV_TOKEN") != "1":
            raise TokenError(
                "The developer token provider is disabled. Set WEBSENTRY_ALLOW_DEV_TOKEN=1 "
                "to use it -- and only on a throwaway repository."
            )
        token = os.environ.get("WEBSENTRY_GITHUB_DEV_TOKEN") or ""
        if not token:
            raise TokenError("WEBSENTRY_GITHUB_DEV_TOKEN is not set.")
...
def default_provider() -> TokenProvider:
    ...
    """
    if os.environ.get("WEBSENTRY_ALLOW_DEV_TOKEN") == "1" and os.environ.get(
        "WEBSENTRY_GITHUB_DEV_TOKEN"
    ):
