...
_TIER_NOTE = {
    1: "This fix is deterministic — WebSentry generated it from a fixed rule, not from a model.",
    2: "**Review required.** This fix is deterministic, but the correct result depends on "
    "context WebSentry cannot see. Read the diff before merging.",
}


def branch_name(scan_id: str, timestamp: int) -> str:
    """`websentry/fix-<8 hex chars of scan id>-<epoch seconds>`.
    ...
    """
    return f"websentry/fix-{scan_id[:8]}-{timestamp}"
...
def commit_message(findings: list[Finding]) -> str:
    ...
    if len(findings) == 1:
        ...
        subject = f"fix(security): resolve {len(findings)} findings from a WebSentry scan"

    body_lines = [f"- {finding.title} ({finding.id})" for finding in findings]
    return f"{subject}\n\n" + "\n".join(body_lines) + "\n\nApplied by WebSentry.\n"


def pull_request_title(findings: list[Finding]) -> str:
    if len(findings) == 1:
        return f"WebSentry: {findings[0].title}"
    return f"WebSentry: {len(findings)} security fixes"
...
def pull_request_body(
    ...
    header = [
        "## Automated security fix from WebSentry",
        "",
        "WebSentry scanned this repository and produced the change below with a "
    ...
    footer = [
        ...
        "### Before you merge",
        "",
        "- Read the diff. WebSentry opened a pull request precisely so that a human "
        "sees the change before it lands — it has no ability to merge this itself.",
        "- Run your test suite. WebSentry did not run it.",
