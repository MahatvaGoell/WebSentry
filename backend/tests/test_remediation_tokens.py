...
def configured_app(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("WEBSENTRY_ALLOW_DEV_TOKEN", raising=False)
    monkeypatch.delenv("WEBSENTRY_GITHUB_DEV_TOKEN", raising=False)
    return key_path
...
async def test_dev_token_refuses_without_the_explicit_opt_in(monkeypatch, mock_api):
    monkeypatch.setenv("WEBSENTRY_GITHUB_DEV_TOKEN", "ghp_something")
    monkeypatch.delenv("WEBSENTRY_ALLOW_DEV_TOKEN", raising=False)
    client = mock_api({})
    with pytest.raises(TokenError, match="WEBSENTRY_ALLOW_DEV_TOKEN"):
...
async def test_dev_token_works_with_both_switches_on(monkeypatch, mock_api):
    monkeypatch.setenv("WEBSENTRY_GITHUB_DEV_TOKEN", "ghp_something")
    monkeypatch.setenv("WEBSENTRY_ALLOW_DEV_TOKEN", "1")
    client = mock_api({})
...
def test_default_provider_is_the_app_unless_both_dev_switches_are_on(monkeypatch):
    ...
    monkeypatch.setenv("WEBSENTRY_GITHUB_DEV_TOKEN", "ghp_x")
