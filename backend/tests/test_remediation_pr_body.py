...
commit, and pull request. CLAUDE.md's remediation rule 9 ("every PR says what
...
def test_title_is_singular_or_counted():
    assert pull_request_title([_finding("a", "No .gitignore")]) == "WebSentry: No .gitignore"
    assert pull_request_title([_finding("a", "x"), _finding("b", "y")]) == "WebSentry: 2 security fixes"
