"""WebSentry API — the FastAPI application object.
...
# Creates backend/data/websentry.db and brings its schema up to date if it
...
# adds the deployed frontend's real origin once WEBSENTRY_FRONTEND_ORIGIN is
...
# production. WEBSENTRY_FRONTEND_ORIGIN starting with https:// is already the
...
def root() -> dict:
    ...
    return {"service": "WebSentry", "docs": "/docs", "health": "/health"}
...
def auth_login(request: Request) -> RedirectResponse:
    ...
    response.set_cookie(
        "websentry_oauth_state", state, httponly=True, samesite="lax", max_age=600, path="/",
    )
    return response
...
async def auth_callback(request: Request, code: str = "", state: str = "") -> RedirectResponse:
    ...
    expected_state = request.cookies.get("websentry_oauth_state")
    ...
    response.delete_cookie("websentry_oauth_state", path="/")
    response.set_cookie(
...
def auth_install(user: User = Depends(current_user)) -> RedirectResponse:
    ...
    which WebSentry user the resulting installation belongs to, and there is
    ...
    response.set_cookie(
        "websentry_install_state", state, httponly=True, samesite="lax", max_age=600, path="/",
    )
    return response
...
async def auth_install_callback(
    ...
    `state` cookie ties that redirect to this WebSentry session. What the
    ...
    """
    frontend = get_frontend_origin()
    expected_state = request.cookies.get("websentry_install_state")

    def _fail(reason: str) -> RedirectResponse:
        response = RedirectResponse(f"{frontend}/settings?install_error={reason}")
        response.delete_cookie("websentry_install_state", path="/")
        return response
...
def installations_revoke(
    ...
    """Stop WebSentry using one installation. 404 if the caller has no live
    ...
    installed on GitHub until the user removes it there. WebSentry can only
...
def scan_fix_bundle(scan_id: str, user: User = Depends(current_user)) -> Response:
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="websentry-fixes-{scan_id[:8]}.zip"'},
    )
...
async def scan_fix_apply(
    ...
    are created. WebSentry never merges it.
    """
    report = get_scan(scan_id)
    if report is None:
        raise HTTPException(status_code=404, detail=f"Scan {scan_id!r} not found")
...
async def scan_fix_applications(
    ...
    `state` has to reflect GitHub's answer rather than what WebSentry last
    happened to see.
    """
    report = get_scan(scan_id)
    if report is None:
...
async def finding_verify(
    ...
    a fix WebSentry opened has merged, the matching `fix_applications` row
...
async def scan_export(scan_id: str, format_id: str, user: User = Depends(current_user)) -> Response:
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="websentry-{slug}.{exporter.extension}"'
        },
    )
...
async def scan_pdf(report: ScanReport, user: User = Depends(current_user)) -> Response:
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="websentry-{slug}.pdf"'},
    )
